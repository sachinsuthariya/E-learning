...
export class Utils {
  ...
  public static createRandomcode = (length: number, isOTP: boolean) => {
    let code = "";
    let characters =
      "abcdefghijklmnopqrstuvwxyzABCDEFGHIJKLMNOPQRSTUVWXYZ0123456789"; // for referral code generator
    if (isOTP) {
  ...
  public static callApi = async (
    ...
    return new Promise((resolve, reject) => {
      ...
      request(options, (error, response, resBody) => {
        ...
        if (error) {
          Utils.logger.error(
            "[Error in calling third party api from request]",
            error
          );
          return reject(error);
        } else if (response.statusCode !== Constants.SUCCESS_CODE) {
          Utils.logger.error(
            `[Error third party api responded] Status code: ${response.statusCode}`,
            resBody
          );
          const errorMsg = {
            isError: true,
  ...
  public static getImagePath(imagePath: string) {
    return imagePath
      ? {
          url:
