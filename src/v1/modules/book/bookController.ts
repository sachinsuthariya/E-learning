...
export class BookController {
    ...
    public create = async (req: any, res: Response) => {
        try {
            ...
            const response = ResponseBuilder.genSuccessResponse(Constants.SUCCESS_CODE, req.t("SUCCESS"), book);
    ...
    public allBooks = async (req: any, res: Response) => {
        try {
            const getAllBooks = await this.bookUtils.getAllBooks();
            const response = ResponseBuilder.genSuccessResponse(Constants.SUCCESS_CODE, req.t("SUCCESS"), getAllBooks);
            return res.status(response.code).json(response);
        } catch (err) {
            const response = ResponseBuilder.genErrorResponse(Constants.INTERNAL_SERVER_ERROR_CODE, req.t("ERR_INTERNAL_SERVER"));
            return res.status(response.error.code).json(response);
